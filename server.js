...
function insertChildren(reportId, crews, operations, stations, equipment) {
  ...
  for (const item of equipment) equipmentInsert.run(reportId, item.equipment_name || "", item.operating_status || "", item.status_description || "", Number(item.working_hours || 0), Number(item.diesel_liters || 0), item.notes || "");
}

app.get("/api/health", (req, res) => res.json({ ok: true, system: "Minya Landfill System V2", database: "SQLite", version: "2.0.0" }));
...
app.get("/api/reports/:id", (req, res) => {
  try {
    ...
    const crews = db.prepare(`SELECT * FROM crews WHERE report_id = ? ORDER BY id`).all(reportId);
    const operations = db.prepare(`SELECT * FROM operations WHERE report_id = ? ORDER BY id`).all(reportId);
    const stations = db.prepare(`SELECT * FROM transfer_stations WHERE report_id = ? ORDER BY id`).all(reportId);
    const equipment = db.prepare(`SELECT * FROM equipment WHERE report_id = ? ORDER BY id`).all(reportId);
    res.json({ ok: true, report, crews, operations, stations, equipment });
...
app.post("/api/reports", (req, res) => {
  try {
    ...
    const existing = db.prepare(`SELECT id FROM daily_reports WHERE report_date = ?`).get(report_date);
    if (existing) return res.status(409).json({ ok: false, message: "يوجد تقرير محفوظ مسبقًا لنفس التاريخ" });
    const reportNo = generateReportNo(report_date);
    const transaction = db.transaction(() => {
      const result = db.prepare(`INSERT INTO daily_reports (report_date, report_no, weather, temperature, start_time, end_time, total_trucks, total_waste_tons, total_diesel, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(report_date, reportNo, weather || "", Number(temperature || 0), start_time || "", end_time || "", Number(total_trucks || 0), Number(total_waste_tons || 0), Number(total_diesel || 0), notes || "");
      const reportId = result.lastInsertRowid;
      insertChildren(reportId, crews, operations, stations, equipment);
      return { id: reportId, report_no: reportNo };
    });
    res.json({ ok: true, message: "تم حفظ التقرير بنجاح", report: transaction() });
...
app.put("/api/reports/:id", (req, res) => {
  try {
    const reportId = Number(req.params.id);
    const { report_date, weather, temperature, start_time, end_time, total_trucks, total_waste_tons, total_diesel, notes, crews = [], operations = [], stations = [], equipment = [] } = req.body;
    const current = db.prepare(`SELECT * FROM daily_reports WHERE id = ?`).get(reportId);
    if (!current) return res.status(404).json({ ok: false, message: "التقرير غير موجود" });
    if (!report_date) return res.status(400).json({ ok: false, message: "تاريخ التقرير مطلوب" });
    const duplicate = db.prepare(`SELECT id FROM daily_reports WHERE report_date = ? AND id <> ?`).get(report_date, reportId);
    if (duplicate) return res.status(409).json({ ok: false, message: "يوجد تقرير آخر محفوظ بنفس التاريخ" });
    const reportNo = generateReportNo(report_date);
    const transaction = db.transaction(() => {
      db.prepare(`UPDATE daily_reports SET report_date=?, report_no=?, weather=?, temperature=?, start_time=?, end_time=?, total_trucks=?, total_waste_tons=?, total_diesel=?, notes=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(report_date, reportNo, weather || "", Number(temperature || 0), start_time || "", end_time || "", Number(total_trucks || 0), Number(total_waste_tons || 0), Number(total_diesel || 0), notes || "", reportId);
      db.prepare(`DELETE FROM crews WHERE report_id=?`).run(reportId);
      db.prepare(`DELETE FROM operations WHERE report_id=?`).run(reportId);
      db.prepare(`DELETE FROM transfer_stations WHERE report_id=?`).run(reportId);
      db.prepare(`DELETE FROM equipment WHERE report_id=?`).run(reportId);
      insertChildren(reportId, crews, operations, stations, equipment);
    });
    transaction();
    res.json({ ok: true, message: "تم تعديل التقرير بنجاح", report: { id: reportId, report_no: reportNo } });
...
app.delete("/api/reports/:id", (req, res) => {
  try {
    const reportId = Number(req.params.id);
    const report = db.prepare(`SELECT id FROM daily_reports WHERE id=?`).get(reportId);
    if (!report) return res.status(404).json({ ok: false, message: "التقرير غير موجود" });
    db.prepare(`DELETE FROM daily_reports WHERE id=?`).run(reportId);
