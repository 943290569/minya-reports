...
module.exports=function installDriverLicenses(app,{db,requireAuth,requireRole,audit,uploadsDir}){
  ...
  app.get('/api/driver-licenses/:id',requireAuth,(req,res)=>{
    ...
    const events=db.prepare('SELECT * FROM driver_license_events WHERE driver_id=? ORDER BY id DESC').all(x.id);
  ...
  app.put('/api/driver-licenses/:id',requireRole('admin','editor'),(req,res)=>{
    ...
    const old=db.prepare('SELECT * FROM driver_licenses WHERE id=?').get(id);
    ...
    db.prepare(`UPDATE driver_licenses SET name_ar=?,name_en=?,address=?,birth_date=?,card_issue_date=?,first_issue_date=?,expiry_date=?,license_class=?,license_type=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(...vals,id);
    db.prepare(`INSERT INTO driver_license_events(driver_id,event_type,details,created_by) VALUES(?,?,?,?)`).run(id,'تعديل بيانات','تم تحديث بيانات الرخصة',req.user.id);
  ...
  app.post('/api/driver-licenses/:id/renew',requireRole('admin','editor'),(req,res)=>{
    try{
      ...
      if(!old)return res.status(404).json({ok:false,message:'الموظف غير موجود'});
      let imageName=old.image_name,imageOriginal=old.image_original,imageMime=old.image_mime;
      if(b.image_base64){
        const text=String(b.image_base64).replace(/^data:[^;]+;base64,/,'');
        const buf=Buffer.from(text,'base64');
        if(!buf.length||buf.length>8*1024*1024)return res.status(400).json({ok:false,message:'صورة الرخصة غير صالحة أو أكبر من 8MB'});
        const ext=(path.extname(String(b.image_name||''))||'.jpg').replace(/[^.a-zA-Z0-9]/g,'').slice(0,8);
        const newName=`license-${id}-${Date.now()}-${crypto.randomBytes(4).toString('hex')}${ext}`;
        fs.writeFileSync(path.join(dir,newName),buf);
        if(old.image_name){try{fs.unlinkSync(path.join(dir,path.basename(old.image_name)));}catch{}}
        imageName=newName;
        imageOriginal=String(b.image_name||'رخصة');
        imageMime=String(b.image_mime||'image/jpeg');
      }
      db.prepare(`UPDATE driver_licenses SET card_issue_date=?,first_issue_date=?,expiry_date=?,license_class=?,license_type=?,image_name=?,image_original=?,image_mime=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(String(b.card_issue_date||old.card_issue_date||''),String(b.first_issue_date||old.first_issue_date||''),String(b.expiry_date||old.expiry_date||''),String(b.license_class||old.license_class||''),String(b.license_type||old.license_type||''),imageName,imageOriginal,imageMime,id);
      db.prepare(`INSERT INTO driver_license_events(driver_id,event_type,details,created_by) VALUES(?,?,?,?)`).run(id,'تجديد رخصة',`تاريخ الانتهاء الجديد: ${b.expiry_date||old.expiry_date||'-'}`,req.user.id);
  ...
  app.delete('/api/driver-licenses/:id/image',requireRole('admin','editor'),(req,res)=>{
    ...
    db.prepare(`INSERT INTO driver_license_events(driver_id,event_type,details,created_by) VALUES(?,?,?,?)`).run(id,'حذف مرفق','تم حذف صورة الرخصة',req.user.id);
    res.json({ok:true});
  });

  const importMap={
    'IMG-20260706-WA0032.jpg':'سعيد ربعي',
    'IMG-20260706-WA0035.jpg':'فادي جبارين',
    'IMG-20260706-WA0036.jpg':'نادي الفروخ',
    'IMG-20260706-WA0040.jpg':'عامر شلالدة',
    'IMG-20260706-WA0047.jpg':'عبد الفتاح فروخ',
    'IMG-20260706-WA0052.jpg':'رمزي البربري',
    'IMG-20260706-WA0055.jpg':'نديم الكوازبة',
    'IMG-20260706-WA0057.jpg':'قصي جبارين',
    'IMG-20260706-WA0058.jpg':'ينال شلالدة',
    'IMG-20260706-WA0059.jpg':'حسن علامه',
    'IMG-20260706-WA0060.jpg':'وحيد أبو عياش',
    'IMG-20260706-WA0061.jpg':'أنس أبو عمر',
    'IMG-20260706-WA0062.jpg':'رامي مطور',
    'IMG-20260706-WA0063.jpg':'محمد جبرين',
    'IMG-20260706-WA0064.jpg':'باسل جرادات',
    'IMG-20260706-WA0067.jpg':'محمود العمور',
    'IMG-20260706-WA0068.jpg':'إسماعيل الفروخ',
    'IMG-20260707-WA0002.jpg':'سامي فروخ',
    'IMG-20260707-WA0009.jpg':'هاني الجنيدي',
    'IMG-20260707-WA0010.jpg':'خالد الكوازبة',
    'IMG-20260707-WA0012.jpg':'جهاد الكوازبة',
    'IMG-20260707-WA0013.jpg':'يزن الكوازبة',
    'IMG-20260707-WA0014.jpg':'أحمد ربعي',
    'IMG-20260707-WA0016.jpg':'عدي الكوازبة',
    'IMG-20260707-WA0017.jpg':'أمجد جبارين',
    'IMG-20260707-WA0021.jpg':'علاء الفروخ',
    'IMG-20260707-WA0023.jpg':'سامي الفروخ',
    'IMG-20260709-WA0008.jpg':'صخر عرامين'
  };

  app.post('/api/driver-licenses/import-zip',requireRole('admin','editor'),express.raw({type:['application/zip','application/x-zip-compressed','application/octet-stream'],limit:'12mb'}),(req,res)=>{
    try{
      if(!Buffer.isBuffer(req.body)||!req.body.length)return res.status(400).json({ok:false,message:'ملف ZIP مطلوب'});
      const zip=new AdmZip(req.body);
      const entries=new Map(zip.getEntries().filter(e=>!e.isDirectory).map(e=>[path.basename(e.entryName),e]));
      const replace=String(req.query.replace||'1')!=='0';
      const imported=[],missing=[],skipped=[];
      const tx=db.transaction(()=>{
        for(const [fileName,nameAr] of Object.entries(importMap)){
          const entry=entries.get(fileName);
          if(!entry){missing.push(fileName);continue;}
          const driver=db.prepare('SELECT * FROM driver_licenses WHERE name_ar=?').get(nameAr);
          if(!driver){skipped.push(`${nameAr}: السجل غير موجود`);continue;}
          if(driver.image_name&&!replace){skipped.push(`${nameAr}: يوجد مرفق سابق`);continue;}
          const buf=entry.getData();
          if(!buf.length||buf.length>8*1024*1024){skipped.push(`${nameAr}: ملف غير صالح أو كبير`);continue;}
          const ext=(path.extname(fileName)||'.jpg').toLowerCase();
          if(!['.jpg','.jpeg','.png','.webp'].includes(ext)){skipped.push(`${nameAr}: نوع صورة غير مدعوم`);continue;}
          ...
          db.prepare(`UPDATE driver_licenses SET image_name=?,image_original=?,image_mime=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(stored,fileName,mime,driver.id);
          db.prepare(`INSERT INTO driver_license_events(driver_id,event_type,details,created_by) VALUES(?,?,?,?)`).run(driver.id,'رفع مرفق','تم رفع صورة الرخصة من ملف ZIP الجماعي',req.user.id);
          imported.push(nameAr);
        }
      });
      tx();
      audit?.(req.user,'IMPORT_DRIVER_LICENSE_IMAGES','driver_license',0,`تم رفع ${imported.length} صورة`);
      res.json({ok:true,message:`تم رفع ${imported.length} صورة رخصة بنجاح`,imported_count:imported.length,imported,missing,skipped});
    }catch(e){
      res.status(400).json({ok:false,message:'تعذر قراءة ملف ZIP أو استيراد الصور',error:e.message});
    }
  });
};
