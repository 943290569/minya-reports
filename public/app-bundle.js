...
const MINYA_ASSET_VERSION = "3.8.0-20260913-cloud-files-v1-mobile-ui3-brand1-header8-20260916-desktop-menu-v6";
...
(function(){
  ...
  async function contractsPage(){
    ...
    el('managementContent').innerHTML=`<div id="managementEditor" class="v3-panel"><h3>إضافة أو تعديل عقد</h3><div class="v3-form-grid">${field('اسم المقاول','contractorName')}${field('رقم العقد','contractNumber')}${field('موضوع العقد','contractTitle')}${field('مسؤول المتابعة','contractOwner')}${field('تاريخ البداية','contractStart','date')}${field('تاريخ النهاية','contractEnd','date')}${field('قيمة العقد','contractValue','number','min="0" step="0.01"')}${field('المدفوع','contractPaid','number','min="0" step="0.01"')}${field('نسبة الإنجاز','contractProgress','number','min="0" max="100"')}${select('الحالة','contractStatus',['فعال','مسودة','متوقف','منتهي','مستلم'])}${field('مستند أو مرفق حتى 5MB','contractFile','file','accept="image/*,.pdf,.doc,.docx,.xls,.xlsx"')}</div><label>ملاحظات<textarea id="contractNotes" rows="3"></textarea></label><div class="management-actions"><button id="managementSave" class="v3-primary">حفظ</button><button id="managementCancel">إلغاء التعديل</button><span id="managementMsg"></span></div></div><div class="v3-panel"><div class="v3-filter"><input id="managementSearch" placeholder="بحث بالمقاول أو العقد"><select id="managementFilter"><option value="">كل الحالات</option><option>فعال</option><option>مسودة</option><option>متوقف</option><option>منتهي</option><option>مستلم</option></select><button id="managementLoad">تحديث</button><button id="managementPrint">طباعة</button></div><div id="managementKpis" class="v3-kpis"></div><div class="v3-table-wrap"><table class="v3-table"><thead><tr><th>المقاول</th><th>العقد</th><th>المدة</th><th>القيمة</th><th>المدفوع</th><th>المتبقي</th><th>الإنجاز</th><th>الحالة</th><th>المرفقات</th><th>إجراء</th></tr></thead><tbody id="managementBody"></tbody></table></div><div id="managementFiles"></div></div>`;
    if(!editable)el('managementEditor').hidden=true;
