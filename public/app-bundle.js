...
(function(){
  ...
  function mountNav(){const nav=document.querySelector('.top-header nav');if(!nav)return;[['/tasks','المهام'],['/contracts','العقود'],['/cells','الخلايا']].forEach(([href,label])=>{if(nav.querySelector(`a[href="${href}"]`))return;const a=document.createElement('a');a.href=href;a.className='app-nav-link';a.textContent=label;nav.appendChild(a);});}
...
(function(){
  const path=(location.pathname.replace(/\/+$/,'')||'/');
  const pageMap={'/':'home','/report':'report','/archive':'archive','/monthly':'monthly','/annual':'annual','/equipment':'equipment','/weekly':'weekly','/search':'search','/managerial':'managerial','/admin':'admin','/reviews':'reviews','/fleet':'fleet','/maintenance-incidents':'incidents','/environment':'environment','/tasks':'tasks','/contracts':'contracts','/cells':'cells','/global-search':'global-search','/ops-dashboard':'ops-dashboard'};
