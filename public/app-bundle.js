...
/* ===== js/app-station-subsource-reports.js ===== */
/* =========================================================
...
/* ===== js/app-equipment-management.js ===== */
(function(){
...
(function(){
  ...
  function reportAge(date){const days=Math.floor((new Date(localDate(new Date()))-new Date(String(date).slice(0,10)))/86400000);return Number.isFinite(days)?`آخر تحديث منذ ${Math.max(0,days)} يوم`:'تاريخ التحديث غير متاح';}
...
(function(){
  ...
  async function dashboard(){
    ...
    c.innerHTML=`<div id="opsKpis" class="v3-kpis"></div><div class="v3-panel"><h3>التنبيهات</h3><div id="opsAlerts"></div></div><div class="v3-panel"><h3>الأقسام</h3><div class="dashboard-grid"><a class="dashboard-card" href="/fleet"><h3>المركبات والسائقون</h3><p>الرخص والتأمين وحركة مركبات المكب.</p></a><a class="dashboard-card" href="/maintenance-incidents"><h3>الصيانة والحوادث</h3><p>الأعطال والحوادث والمتابعة والتكاليف.</p></a><a class="dashboard-card" href="/environment"><h3>العصارة والغطاء</h3><p>العصارة ونقلات الصهريج والغطاء اليومي.</p></a><a class="dashboard-card" href="/tasks"><h3>الملاحظات والمهام</h3><p>التكليفات والمواعيد ونسب الإنجاز.</p></a><a class="dashboard-card" href="/contracts"><h3>المقاولون والعقود</h3><p>القيم والدفعات والإنجاز ومدة العقد.</p></a><a class="dashboard-card" href="/cells"><h3>الخلايا والسعة</h3><p>السعة المستخدمة والمتبقية والعمر التشغيلي.</p></a><a class="dashboard-card" href="/global-search"><h3>البحث الشامل</h3><p>بحث موحد داخل بيانات النظام.</p></a></div></div>`;
    const [f,i,e,r,m]=await Promise.all([api('/api/ops/fleet').catch(()=>({vehicles:[]})),api('/api/incidents').catch(()=>({incidents:[]})),api(`/api/ops/environment?from=${from}&to=${today}`).catch(()=>({totals:{},rows:[]})),api('/api/reports').catch(()=>({reports:[]})),api('/api/ops/management-dashboard').catch(()=>({alerts:[],summary:{}}))]);
    ...
    (m.alerts||[]).forEach(x=>alerts.push(`${x.source} - ${x.title}: ${x.detail}`));
