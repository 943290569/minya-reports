/* V30: robust Pivot parser. Supports landfill pivots where date rows themselves carry quantity/count, plus nested category rows. */
(function(){
  ...
  async function pivot(file){
    ...
    for(const name of wb.SheetNames){
      ...
      for(let h=0;h<Math.min(rows.length,40);h++){
        ...
        const okA=a.includes('row labels')||a.includes('تسميات')||a.includes('التاريخ');
        const okB=b.includes('كمي')||b.includes('مجموع')||b.includes('طن');
        const okC=c.includes('عدد')||c.includes('وحد')||c.includes('مركبات')||c.includes('شاحنات');
        if(okA&&okB&&okC){if(!best||rows.length>best.rows.length)best={rows,h};break;}
      }
    }
    return best;
  }
  ...
  async function parseLand(file){
    const p=await pivot(file),map=new Map();if(!p)return map;const y=inferYear(p.rows,p.h);
    let section=null;
    const finish=()=>{
      if(!section)return;
      ...
      const x=emptyLand();
      let childCount=0;
      for(const row of children){const label=clean(row[0]);if(!label||/الاجمالي|grand total/i.test(norm(label)))continue;const tons=num(row[1]),trucks=num(row[2]);if(tons===0&&trucks===0)continue;const k=bucket(label);x[k].tons+=tons;x[k].trucks+=trucks;childCount++;}
      /* Flat Pivot: the date row itself is the actual landfill daily value. */
      if(childCount===0&&(dateTons!==0||dateTrucks!==0)){x.other.tons+=dateTons;x.other.trucks+=dateTrucks;}
      map.set(date,x);section=null;
    };
    for(let r=p.h+1;r<p.rows.length;r++){
      const row=p.rows[r]||[],date=isoRaw(row[0],y);
      if(date){finish();section={date,dateTons:num(row[1]),dateTrucks:num(row[2]),children:[]};continue;}
  ...
  function schedule(){timers.splice(0).forEach(clearTimeout);[300,800,1600,2800,4500,7000,10000].forEach(ms=>timers.push(setTimeout(()=>apply().catch(console.error),ms)));}
