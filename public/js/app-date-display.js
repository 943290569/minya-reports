...
(function(){
  ...
  function applyChartMonths(root=document){root.querySelectorAll?.(".executive-trend-item small").forEach((el,index)=>{if(index<12)el.textContent=shortMonths[index];});root.querySelectorAll?.(".modern-chart-eyebrow,#annualBestMonth,#annualWorstMonth").forEach(el=>{el.textContent=arabicNameToShort(el.textContent);});}
