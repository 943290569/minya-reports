/* V31: exact flat-Pivot landfill reader. Captures the original file before normalizers replace it, reads Row Labels + quantity + vehicle count, and writes daily landfill values + totals. */
(function(){
  ...
  function capture(){
    const f=$('sourceFile_landfill')?.files?.[0];
    if(f&&!/normalized/i.test(f.name))original=f;
  }

  async function parse(){
    if(!original)return new Map();
    const wb=XLSX.read(await original.arrayBuffer(),{type:'array',cellDates:false,cellStyles:false,cellNF:false,cellHTML:false});
    let best=null;
    for(const name of wb.SheetNames){
      ...
      for(let h=0;h<Math.min(rows.length,20);h++){
        ...
        if((a.includes('row labels')||a.includes('تسميات'))&&(b.includes('كمي')||b.includes('مجموع'))&&(c.includes('عدد')||c.includes('مركبات')||c.includes('شاحنات'))){best={rows,h};break;}
      }
      if(best)break;
    }
    const out=new Map();
    ...
    for(let r=best.h+1;r<best.rows.length;r++){
      ...
      if(!d)continue;
      out.set(d,{tons:num(row[1]),trucks:Math.max(0,Math.round(num(row[2])))});
    }
    return out;
  }
  ...
  async function apply(){
    ...
    const mp=await parse();if(!mp.size)return;
    let total={tons:0,trucks:0};
    for(const v of mp.values()){total.tons+=v.tons;total.trucks+=v.trucks;}
    table.querySelectorAll('tbody tr').forEach(tr=>{
      const c=tr.querySelectorAll('td');if(c.length<15)return;
      const d=clean(c[0]?.textContent),v=mp.get(d);if(!v)return;
      /* This uploaded Pivot is filtered to مكب المنيا نفايات هيئات محلية, so place the value in local-authorities and total landfill columns. */
      if(c[6])c[6].textContent=text(v);
      if(c[10])c[10].textContent=text(v);
    });
    setCard(root,'مجموع وارد المكب',text(total));
    setCard(root,'نفايات واردة للمكب',text(total));
  }
  ...
  function init(){
    document.addEventListener('change',e=>{if(e.target?.id==='sourceFile_landfill'){capture();}},true);
    const btn=$('analyzeSourceFilesBtn');if(btn)btn.addEventListener('click',()=>{capture();schedule();},true);
  }
