...
(function(){
  ...
  function apply(){
    ...
    let box=root.querySelector('#pivotTotalsV18');if(!box){box=document.createElement('div');box.id='pivotTotalsV18';box.className='source-import-summary';root.prepend(box);}
    const html=`<div><span>مجموع وارد المكب</span><strong>${text(lt)}</strong></div><div><span>مجموع المحطات</span><strong>${text(stationAll)}</strong></div><div><span>مجموع عبد العزيز</span><strong>${text(at)}</strong></div>`;if(box.innerHTML!==html)box.innerHTML=html;

    const summaries=[...root.querySelectorAll('.source-import-summary')].filter(x=>x.id!=='pivotTotalsV18'&&!x.closest('#specialCategoriesV16'));
    const core=summaries.find(x=>x.querySelectorAll(':scope > div').length>=5);
    const cards=core?.querySelectorAll(':scope > div');
    if(cards?.[2]){const s=cards[2].querySelector('strong');if(s)s.textContent=text(stationAll);}
    if(cards?.[3]){const s=cards[3].querySelector('strong');if(s)s.textContent=text(at);}
