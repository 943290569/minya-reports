(function(){
  ...
  const bootstrapMap={
    'IMG-20260706-WA0032.jpg':'سعيد ربعي','IMG-20260706-WA0035.jpg':'فادي جبارين','IMG-20260706-WA0036.jpg':'نادي الفروخ','IMG-20260706-WA0040.jpg':'عامر شلالدة','IMG-20260706-WA0047.jpg':'عبد الفتاح فروخ','IMG-20260706-WA0052.jpg':'رمزي البربري','IMG-20260706-WA0055.jpg':'نديم الكوازبة','IMG-20260706-WA0057.jpg':'قصي جبارين','IMG-20260706-WA0058.jpg':'ينال شلالدة','IMG-20260706-WA0059.jpg':'حسن علامه','IMG-20260706-WA0060.jpg':'وحيد أبو عياش','IMG-20260706-WA0061.jpg':'أنس أبو عمر','IMG-20260706-WA0062.jpg':'رامي مطور','IMG-20260706-WA0063.jpg':'محمد جبرين','IMG-20260706-WA0064.jpg':'باسل جرادات','IMG-20260706-WA0067.jpg':'محمود العمور','IMG-20260706-WA0068.jpg':'إسماعيل الفروخ','IMG-20260707-WA0002.jpg':'سامي فروخ','IMG-20260707-WA0009.jpg':'هاني الجنيدي','IMG-20260707-WA0010.jpg':'خالد الكوازبة','IMG-20260707-WA0012.jpg':'جهاد الكوازبة','IMG-20260707-WA0013.jpg':'يزن الكوازبة','IMG-20260707-WA0014.jpg':'أحمد ربعي','IMG-20260707-WA0016.jpg':'عدي الكوازبة','IMG-20260707-WA0017.jpg':'أمجد جبارين','IMG-20260707-WA0021.jpg':'علاء الفروخ','IMG-20260707-WA0023.jpg':'سامي الفروخ','IMG-20260709-WA0008.jpg':'صخر عرامين'
  };
  ...
  function extractIdentity(text){
    ...
    const matches=raw.match(/\d[\d\s.\-]{7,18}\d/g)||[];
    const ids=[...new Set(matches.map(x=>x.replace(/\D/g,'')).filter(x=>x.length===9))];
    const checked=ids.filter(validId9);
    if(checked.length===1)return checked[0];
    if(ids.length===1)return ids[0];
    return '';
  }
  async function imageCanvas(file,maxSide=1800){const bitmap=await createImageBitmap(file),scale=Math.min(1,maxSide/Math.max(bitmap.width,bitmap.height)),w=Math.max(1,Math.round(bitmap.width*scale)),h=Math.max(1,Math.round(bitmap.height*scale)),c=document.createElement('canvas');c.width=w;c.height=h;const ctx=c.getContext('2d',{alpha:false});ctx.fillStyle='#fff';ctx.fillRect(0,0,w,h);ctx.drawImage(bitmap,0,0,w,h);bitmap.close?.();return c;}
  async function compressCanvas(c,target=250*1024){let q=.84,blob=null;for(let i=0;i<9;i++){blob=await new Promise(resolve=>c.toBlob(resolve,'image/webp',q));if(blob&&blob.size<=target)break;q=Math.max(.46,q-.05);}if(!blob)throw new Error('تعذر ضغط الصورة');return blob;}
  async function recognizeIdentity(canvas){
    ...
    return extractIdentity(result?.data?.text||'');
  }
  async function uploadOne(file,index,total){
    const canvas=await imageCanvas(file);
    const identity=await recognizeIdentity(canvas);
    if(!identity)return {ok:false,file:file.name,message:'لم يتم التعرف على رقم الهوية من البند 6'};
    ...
    return {ok:true,file:file.name,identity,name_ar:d.name_ar,before:file.size,after:blob.size,identity_added:d.identity_added};
  }
  async function run(){
    ...
    if(!confirm(`سيتم قراءة رقم الهوية من البند 6 ومطابقة ${files.length} صورة مع الموظفين. متابعة؟`))return;
    ...
    try{
      for(let i=0;i<files.length;i++){
        ...
        let out;try{out=await uploadOne(files[i],i+1,files.length);}catch(e){out={ok:false,file:files[i].name,message:e.message||'فشل المعالجة'};}
      ...
      if(success){window.dispatchEvent(new Event('driver-licenses-updated'));setTimeout(()=>location.reload(),1800);}
