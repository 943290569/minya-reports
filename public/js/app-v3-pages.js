...
(function(){
  ...
  async function renderAdmin(){
    ...
    c.innerHTML=`<div class="v3-admin-grid"><div class="v3-panel"><h3>المستخدمون</h3><div id="usersList"></div><hr><div class="v3-form-grid"><label>اسم المستخدم<input id="uName"></label><label>الاسم الظاهر<input id="uDisplay"></label><label>كلمة المرور<input id="uPass" type="password"></label><label>الصلاحية<select id="uRole"><option value="viewer">قراءة فقط</option><option value="editor">محرر</option><option value="admin">مدير</option></select></label></div><button id="uAdd" class="v3-primary">إضافة مستخدم</button><p id="uMsg"></p></div><div class="v3-panel"><h3>النسخ الاحتياطي والاستعادة</h3><a class="v3-primary inline" href="/api/backup/download">تنزيل نسخة كاملة</a><label class="restore-label">استعادة نسخة JSON<input id="restoreFile" type="file" accept="application/json"></label><button id="restoreBtn">استعادة بعد التأكيد</button><p id="restoreMsg">يتم إنشاء نسخة تلقائية قبل الاستعادة.</p></div></div><div class="v3-panel"><h3>سجل التعديلات Audit Log</h3><div class="v3-table-wrap"><table class="v3-table"><thead><tr><th>الوقت</th><th>المستخدم</th><th>الإجراء</th><th>النوع</th><th>المعرف</th><th>التفاصيل</th></tr></thead><tbody id="auditBody"></tbody></table></div></div>`;
    async function load(){try{const [u,a]=await Promise.all([api('/api/users'),api('/api/audit?limit=300')]);usersList.innerHTML=u.users.map(x=>`<div class="user-row"><div><strong>${esc(x.display_name)}</strong><small>${esc(x.username)} — ${esc(x.role)}</small></div><span>${x.is_active?'نشط':'موقوف'}</span></div>`).join('');auditBody.innerHTML=a.logs.map(x=>`<tr><td>${esc(x.created_at)}</td><td>${esc(x.username)}</td><td>${esc(x.action)}</td><td>${esc(x.entity_type)}</td><td>${esc(x.entity_id)}</td><td>${esc(x.details)}</td></tr>`).join('');}catch(e){c.innerHTML=`<div class="v3-panel">${esc(e.message)} — هذه الصفحة للمدير فقط.</div>`;}}
    uAdd.onclick=async()=>{try{await api('/api/users',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:uName.value,display_name:uDisplay.value,password:uPass.value,role:uRole.value})});uMsg.textContent='تمت إضافة المستخدم';load();}catch(e){uMsg.textContent=e.message;}};
    restoreBtn.onclick=async()=>{const f=restoreFile.files[0];if(!f)return; if(!confirm('سيتم استبدال بيانات التقارير الحالية بالنسخة المختارة. متابعة؟'))return;try{const obj=JSON.parse(await f.text());const d=await api('/api/backup/restore',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(obj)});restoreMsg.textContent=d.message;}catch(e){restoreMsg.textContent=e.message;}};load();
  }
