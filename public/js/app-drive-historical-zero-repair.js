(()=>{
  ...
  const setState=(msg,kind='')=>{const el=document.getElementById('driveConnectionState');if(!el)return;el.textContent=msg;el.className=`drive-state${kind?` ${kind}`:''}`};
  ...
  function parseSheet(name,sheet){
    ...
    const operations=[];const opStart=findRow(rows,'موقع مكب المنيا'),stStart=findRow(rows,'موقع محطات الترحيل');if(opStart>=0){const end=stStart>opStart?stStart:rows.length;for(let i=opStart+1;i<end;i++){const r=rows[i],n=text(r[0]);if(!n)continue;const nn=norm(n);if(nn.includes('كميات النفايات الوارده')||nn.includes('المجموع النهائي'))break;operations.push({operation_name:n,vehicle_count:num(r[3]),quantity:num(r[4]),unit:unit(r[5]),notes:''})}}
    ...
    return{report_date,weather,temperature,start_time:'04:00',end_time:'19:00',total_trucks,total_waste_tons,total_diesel,notes:'',crews,operations,stations,equipment};
  }
  ...
  async function allDriveFiles(){let files=[],pageToken='';for(let page=0;page<5;page++){const q=encodeURIComponent(`trashed=false and (mimeType='${SHEET_MIME}' or mimeType='${XLSX_MIME}')`);const fields=encodeURIComponent('nextPageToken,files(id,name,mimeType,modifiedTime,size)');const url=`https://www.googleapis.com/drive/v3/files?q=${q}&fields=${fields}&orderBy=modifiedTime%20desc&pageSize=100${pageToken?`&pageToken=${encodeURIComponent(pageToken)}`:''}`;const d=await driveJson(url);files.push(...(d.files||[]));pageToken=d.nextPageToken||'';if(!pageToken)break}return files}
  ...
  window.fetch=async function(input,init){
    ...
    const auth=headers.get('Authorization')||headers.get('authorization')||'';if(url.includes('www.googleapis.com/drive/v3/')&&auth.startsWith('Bearer '))driveToken=auth.slice(7);
    const res=await originalFetch(input,init);
    if(driveToken&&url.includes('www.googleapis.com/drive/v3/files?')&&res.ok)setTimeout(runRepair,300);
    return res;
  };
})();