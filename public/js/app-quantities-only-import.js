...
(function(){
  ...
  function optional(m,names){for(const name of names){if(m[name]!==undefined)return m[name];}return-1;}
  ...
  async function parse(file){
    ...
    const cols={date:need(m,'التاريخ'),landfillCount:need(m,'مكب المنيا - عدد المركبات'),landfillQty:need(m,'مكب المنيا - الكمية (طن)'),tarqCount:need(m,'ترقوميا - عدد الشاحنات'),tarqQty:need(m,'ترقوميا - الكمية (طن)'),yataCount:need(m,'يطا - عدد الشاحنات'),yataQty:need(m,'يطا - الكمية (طن)'),hebCount:need(m,'الخليل - عدد الشاحنات'),hebQty:need(m,'الخليل - الكمية (طن)'),leachCount:need(m,'العصارة - عدد المركبات'),leachQty:need(m,'العصارة - الكمية'),extCount:need(m,'طمم خارجي - عدد المركبات'),extQty:need(m,'طمم خارجي - الكمية'),intCount:need(m,'طمم داخلي - عدد المركبات'),intQty:need(m,'طمم داخلي - الكمية'),styleCount:need(m,'أسلوب - عدد المركبات'),styleQty:need(m,'أسلوب - الكمية'),sortCount:need(m,'خط الفرز - عدد المركبات'),sortQty:need(m,'خط الفرز - الكمية'),azizCount:optional(m,['عبد العزيز - عدد النقلات','عبد العزيز - عدد الشاحنات','عزيز - عدد النقلات','عزيز - عدد الشاحنات']),azizQty:optional(m,['عبد العزيز - الكمية (طن)','عبد العزيز - الكمية','عزيز - الكمية (طن)','عزيز - الكمية'])};
    const days=[];
    for(let i=h+1;i<rows.length;i++){
      const r=rows[i]||[],date=iso(r[cols.date]);if(!date)continue;
      days.push({date,landfillCount:num(r[cols.landfillCount]),landfillQty:num(r[cols.landfillQty]),tarqCount:num(r[cols.tarqCount]),tarqQty:num(r[cols.tarqQty]),yataCount:num(r[cols.yataCount]),yataQty:num(r[cols.yataQty]),hebCount:num(r[cols.hebCount]),hebQty:num(r[cols.hebQty]),leachCount:num(r[cols.leachCount]),leachQty:num(r[cols.leachQty]),extCount:num(r[cols.extCount]),extQty:num(r[cols.extQty]),intCount:num(r[cols.intCount]),intQty:num(r[cols.intQty]),styleCount:num(r[cols.styleCount]),styleQty:num(r[cols.styleQty]),sortCount:num(r[cols.sortCount]),sortQty:num(r[cols.sortQty]),azizCount:cols.azizCount>=0?num(r[cols.azizCount]):0,azizQty:cols.azizQty>=0?num(r[cols.azizQty]):0,hasAzizColumns:cols.azizCount>=0&&cols.azizQty>=0});
    }
  ...
  function totals(days){return days.reduce((a,d)=>({landfill:a.landfill+d.landfillQty,stations:a.stations+d.tarqQty+d.yataQty+d.hebQty,aziz:a.aziz+d.azizQty,azizCount:a.azizCount+d.azizCount,external:a.external+d.extQty,internal:a.internal+d.intQty,leachate:a.leachate+d.leachQty,sorting:a.sorting+d.sortQty}),{landfill:0,stations:0,aziz:0,azizCount:0,external:0,internal:0,leachate:0,sorting:0});}
  function render(days){
    const root=$('sourceFilesPreview'),msg=$('sourceFilesMessage');if(!root)return;const t=totals(days);
    root.innerHTML=`<div class="source-import-summary"><div><span>الأيام المقروءة</span><strong>${days.length}</strong></div><div><span>نفايات المكب</span><strong>${fmt(t.landfill)} طن</strong></div><div><span>المحطات</span><strong>${fmt(t.stations)} طن</strong></div><div><span>عبد العزيز / ضمن يطا</span><strong>${fmt(t.azizCount)} نقلة · ${fmt(t.aziz)} طن</strong></div><div><span>طمم خارجي</span><strong>${fmt(t.external)}</strong></div><div><span>طمم داخلي</span><strong>${fmt(t.internal)}</strong></div></div><div class="drive-preview-note"><strong>ملف الكميات الموحد:</strong> يطا تتضمن عبد العزيز ضمن إجماليها. إذا وُجد عمودا عبد العزيز فسيتم حفظهما كتفصيل مستقل فقط دون إضافتهما مرة ثانية إلى يطا. السولار لا يتم تغييره من هذا الاستيراد.</div><div class="source-import-table-wrap"><table class="v3-table quantities-import-table"><thead><tr><th>التاريخ</th><th>المكب مركبات</th><th>المكب طن</th><th>ترقوميا</th><th>يطا</th><th>عبد العزيز / ضمن يطا</th><th>الخليل</th><th>العصارة</th><th>طمم خارجي</th><th>طمم داخلي</th><th>أسلوب</th><th>خط الفرز</th><th>رش المياه</th></tr></thead><tbody>${days.map(d=>`<tr><td>${esc(d.date)}</td><td>${fmt(d.landfillCount)}</td><td><strong>${fmt(d.landfillQty)}</strong></td><td>${fmt(d.tarqCount)} / ${fmt(d.tarqQty)}</td><td>${fmt(d.yataCount)} / ${fmt(d.yataQty)}</td><td>${fmt(d.azizCount)} / ${fmt(d.azizQty)}</td><td>${fmt(d.hebCount)} / ${fmt(d.hebQty)}</td><td>${fmt(d.leachCount)} / ${fmt(d.leachQty)}</td><td>${fmt(d.extCount)} / ${fmt(d.extQty)}</td><td>${fmt(d.intCount)} / ${fmt(d.intQty)}</td><td>${fmt(d.styleCount)} / ${fmt(d.styleQty)}</td><td>${fmt(d.sortCount)} / ${fmt(d.sortQty)}</td><td>${fmt(d.sprays)} مرة · ${fmt(d.water)} كوب</td></tr>`).join('')}</tbody></table></div><div id="quantitiesApprovalBar" class="source-import-toolbar" style="position:sticky;bottom:8px;z-index:20;background:rgba(255,255,255,.96);padding:12px;border:1px solid #d8e4df;border-radius:12px;box-shadow:0 4px 18px rgba(0,0,0,.08);margin:16px 0"><button id="approveQuantitiesBtn" type="button" class="drive-primary">اعتماد ملف الكميات في التقارير</button><span>يحدّث الكميات والمحطات والعمليات فقط. السولار والمعدات وباقي بيانات التقرير تبقى كما هي.</span></div>`;
  ...
  async function approve(){
    ...
    btn.disabled=true;let created=0,updated=0,failed=0;try{
      ...
      const hasAzizColumns=preview.some(d=>d.hasAzizColumns);
      let azizSaved=0;
      if(hasAzizColumns){
