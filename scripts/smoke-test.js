...
const port=5099, base=`http://127.0.0.1:${port}`, uploadsDir=path.join(process.cwd(),'uploads');
...
(async()=>{try{
 ...
 fs.writeFileSync(path.join(uploadsDir,'manual-orphan.tmp'),'orphan');assert(uploadFiles().includes('manual-orphan.tmp'),'orphan fixture failed');
 x=await json('/api/backup/restore',auth(admin,'POST',backup));assert(x.r.status===200&&x.data.count===1,'restore failed');assert(!uploadFiles().includes('manual-orphan.tmp'),'restore did not clean orphan file');
