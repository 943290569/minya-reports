...
const newPages='const appPages=["/report","/archive","/monthly","/annual","/equipment","/weekly","/search","/managerial","/reviews","/admin","/ops-dashboard","/fleet","/maintenance-incidents","/environment","/tasks","/contracts","/cells","/global-search"];';
if(server.includes(oldPages))server=server.replace(oldPages,newPages);
else if(server.includes('const appPages=')&&!server.includes('"/tasks"'))server=server.replace(/const appPages=\[[^;]+\];/,newPages);
