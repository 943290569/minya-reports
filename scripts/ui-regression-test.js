...
assert(appearance.includes('typographyPreset: ["compact", "balanced", "large", "accessible", "custom"]'), "typography presets are incomplete");
assert(appearance.includes('data-appearance-key="navFontSize"'), "navigation font control is missing");
assert(appearance.includes('data-appearance-key="headingFontSize"'), "heading font control is missing");
assert(appearance.includes('data-appearance-key="metricFontSize"'), "metric font control is missing");
assert(appearance.includes('data-appearance-key="smallFontSize"'), "supporting text control is missing");
assert(appearance.includes('data-appearance-key="lineHeight"'), "line-height control is missing");
assert(appearance.includes('data-appearance-key="fontFamily"'), "font family control is missing");
assert(appearance.includes('data-appearance-key="fontWeight"'), "font weight control is missing");
assert(appearance.includes('typographyKeys.includes(key)'), "typography sliders are not saved as custom settings");
assert(appearance.includes('minyaTypographyRuntimeStyles'), "typography overrides are not mounted after page styles");
...
assert(server.includes('const TYPOGRAPHY_PRESETS'), "server typography presets are missing");
assert(server.includes('typographyRevision: 2'), "server typography migration is missing");
...
assert(read("public/external-diesel.html").includes("js/app-header-menu.js?v=desktop-menu-v4"), "company diesel page does not load the complete navigation");
assert(read("public/drivers-licenses.html").includes("js/app-header-menu.js?v=desktop-menu-v4"), "drivers page does not load the complete navigation");
...
assert(read("public/system.html").includes('app-bundle.css?v=3.5.0-20260911-typography-v2-wa8'), "system page is not using the current typography styles");
