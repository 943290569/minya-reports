...
assert(health.includes("integrity?.level==='danger'"),'system health dashboard does not handle danger integrity state');
assert(health.includes("integrity?.level==='warning'"),'system health dashboard does not handle warning integrity state');
assert(health.includes("text:'سليمة'"),'system health dashboard does not expose the healthy state');
assert(health.includes('صحة النظام:'),'compact system-health label is missing');
...
console.log('V3.5 regression checks passed: compact admin health + strict attachments + safe backup/restore + isolated Drive parser.');
